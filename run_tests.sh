...
# Run the tests
docker run -e "browser=chrome" -t --rm --network cyberdojoselenium_default cyberdojo/system-tests
CHROME_RESULT=$?

docker run -e "browser=firefox" -t --rm --network cyberdojoselenium_default cyberdojo/system-tests
FIREFOX_RESULT=$?
